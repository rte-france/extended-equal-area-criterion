"""
Module for ech_file_parser.

:module: ech_file_parser
"""
...
from deeac.IO.eurostag.topology.record_description import RecordDescription, RecordType
...
class EchRecordType(RecordType):
    ...
    :cvar START_OF_FILE: Value for start of file.
    :cvar GENERAL_PARAMETERS: Value for general parameters.
    :cvar SPECIAL_PARAMETER: Value for special parameter.
    :cvar SPECIFIC_COMMENT: Value for specific comment.
    :cvar GENERAL_COMMENT: Value for general comment.
    :cvar AC_ZONE: Value for ac zone.
    :cvar NODE: Value for node.
    :cvar SLACK_BUS: Value for slack bus.
    :cvar LINE: Value for line.
    :cvar COUPLING_DEVICE: Value for coupling device.
    :cvar GENERATOR: Value for generator.
    :cvar TYPE1_TRANSFORMER: Value for type1 transformer.
    :cvar TYPE2_TRANSFORMER: Value for type2 transformer.
    :cvar TYPE4_TRANSFORMER: Value for type4 transformer.
    :cvar TYPE8_TRANSFORMER: Value for type8 transformer.
    :cvar TYPE9_TRANSFORMER: Value for type9 transformer.
    :cvar DISSYMMETRICAL_BRANCHE: Value for dissymmetrical branche.
    :cvar SERIAL_BRANCHE: Value for serial branche.
    :cvar LOAD: Value for load.
    :cvar DC_ZONE: Value for dc zone.
    :cvar DC_NODE: Value for dc node.
    :cvar DC_LINK: Value for dc link.
    :cvar SVC: Value for svc.
    :cvar CAPACITOR_BANK: Value for capacitor bank.
    :cvar HVDC_CSC_CONVERTER: Value for hvdc csc converter.
    :cvar HVDC_VSC_CONVERTER: Value for hvdc vsc converter.
    """
    HEADER = 'HEADER'
    START_OF_FILE = 'B'
...
class EchEurostagFileParser(EurostagFileParser):
    ...
    def __init__(self, file_path: str):
        ...
        self._record_types = {}
        for tpe in EchRecordType:
            self._record_types[tpe.value] = tpe
    ...
    def _get_record_number(self, record_type: RecordType, file_line: str) -> Union[int, None]:
        ...
        :param file_line: The line of the file corresponding to the record.
        ...
        if (record_type != self._prev_record_type or
