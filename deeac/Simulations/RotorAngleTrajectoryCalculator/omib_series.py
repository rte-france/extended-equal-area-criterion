...
from deeac.Simulations.RotorAngleTrajectoryCalculator.omib_calculator import (OMIBRotorAngleTrajectoryCalculator,
                                                                                   OMIBTrajectoryPoint,
                                                                                   OMIBTrajectoryAngle)


def _closest_root(roots: List[float], reference_root: float) -> float:
    """
    Return the candidate root closest to a reference root.
    
    :param roots: Candidate roots.
    :param reference_root: Root used as the distance reference.
    :return: Closest candidate root.
    """
    closest_root = roots[0]
    smallest_distance = abs(closest_root - reference_root)
    for root in roots[1:]:
        distance = abs(root - reference_root)
...
class OMIBTaylorSeries(OMIBRotorAngleTrajectoryCalculator):
    ...
    def _get_trajectory_point(
        ...
        if nb_roots == 0:
            ...
            raise ValueError(to_angle.angle)
        elif nb_roots == 1:
            ...
            if other_root is None:
                ...
                root = _closest_root(max_order_roots, other_root)
