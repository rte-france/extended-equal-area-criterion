...
from deeac.Simulations.OMIB.omib import OMIB, OMIBStabilityState, OMIBSwingState
from deeac.Models.network import NetworkState


def _sort_angles_by_swing(
    omib_angles: List[Tuple[float, Optional[float], NetworkState]],
    swing_factor: int,
) -> List[Tuple[float, Optional[float], NetworkState]]:
    """
    Sort OMIB angle tuples in the active swing direction.
    
    :param omib_angles: Angle tuples to sort.
    :param swing_factor: ``1`` for forward swing, ``-1`` for backward swing.
    :return: Sorted angle tuples.
    """
    keyed_angles: List[Tuple[float, Tuple[float, Optional[float], NetworkState]]] = list()
    for omib_angle in omib_angles:
        keyed_angles.append((omib_angle[0] * swing_factor, omib_angle))
    keyed_angles.sort()
...
class EAC:
    ...
    :ivar omib: omib.
    :ivar angle_increment: angle increment.
    :ivar max_integration_angle: max integration angle.
    :ivar exploration_angle_increment_factor: exploration angle increment factor.
    :ivar exploration_last_angle_increment_factor: exploration last angle increment factor.
    
    Implements the Equal Area Criterion on the OMIB trajectory as described in the EEAC paper:
    stability is assessed by comparing accelerating and decelerating power areas in the
    angle-power plane to find the critical clearing angle (CCA).
    """
    ...
    def _get_trajectory_power_area(self,
                                   from_rotor_angle: float,
                                   to_rotor_angle: float,
                                   state: NetworkState,
                                   angle_shift: float,
                                   constant_electric_power: float,
    ...
    def _get_critical_and_maximum_angles(self) -> Tuple[float, float]:
        ...
        """
        # TODO: Explore if the exploration can be made algorithmically faster
    ...
    def generate_area_plot(self, output_file: str):
        ...
        if not plot_only_initial_electric_powers:
            ...
            angles = []
            sorted_angles = _sort_angles_by_swing(omib_angles, self._swing_factor)
