...
class DOMIB(OMIB):
    """
    Class modeling a Dynamic One Machine Infinite Bus (DOMIB) system based on two sets of generators.
    A DOMIB can consider the deviation of the rotor angle of each generator compared to the Partial
    Center Of Angle (PCOA) of each cluster at specific time instants.
    """
    ...
    def __init__(self, network: Network, critical_cluster: GeneratorCluster, non_critical_cluster: GeneratorCluster):
        ...
        # All generators
        self._generators = critical_cluster.generators.union(non_critical_cluster.generators)

        # Time instants at which the OMIB is updated
        self._update_times: List[Tuple[float, NetworkState]] = []
    ...
    def _get_generator_angular_deviation(
        ...
        try:
            ...
            # Compute deviation
            deviation = generator_cluster.get_generator_angular_deviation(generator_name, time, state)
